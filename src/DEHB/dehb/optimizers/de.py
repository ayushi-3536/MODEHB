...
class DEBase():
    ...
    #Todo: see where it is happening and if you need NDS to do it
    def _sort_pop(self):
...
class DE(DEBase):
    ...
    def crossover_bin(self, target, mutant,seed=1):
    ...
    def crossover_exp(self, target, mutant,seed=1):
    ...
    def crossover(self, target, mutant,seed=1):
        '''Performs DE crossover
        '''
        if self.crossover_strategy == 'bin':
            offspring = self.crossover_bin(target, mutant,seed)
        elif self.crossover_strategy == 'exp':
            offspring = self.crossover_exp(target, mutant,seed)
        return offspring
