...
def input_arguments():
    ...
    parser.add_argument('--min_budget', default=1, type=int, nargs='?',
                        help='minimum budget')
    parser.add_argument('--max_budget', default=200, type=int, nargs='?',
                        help='maximum budget')
    ...
    parser.add_argument('--log_interval', type=int, default=100,
                        help='total runtime')
    parser.add_argument('--timeout', default=23*3600, type=int, help='Timeout in sec. 0 -> no timeout')
...
experiment = get_acc_dsp('MODEHB_Adult_{}_{}_{}_mib_{}_mab_{}'.format(args.seed,args.mo_strategy,
                                                                      args.mo_selection_strategy,args.min_budget,args.max_budget))
...
def adult_objective_function(cfg, budget, run=1, **kwargs):
    global num_evals
    ...
    params['n_fc_l'] = cfg['n_fc_l']

    params['n_fc_0'] = cfg['n_fc_0'] if 'n_fc_0' in cfg.keys() else 16
    params['n_fc_1'] = cfg['n_fc_1'] if 'n_fc_1' in cfg.keys() else 16
    params['n_fc_2'] = cfg['n_fc_2'] if 'n_fc_2' in cfg.keys() else 16
    #
    params['alpha'] = cfg['alpha']
    params['lr'] = cfg['lr']
    params['beta_1'] = cfg['beta_1']
    params['beta_2'] = cfg['beta_2']
    params['tol'] = cfg['tol']
    ...
    num_evals += 1
    dsp = float(data.df[data.df['metric_name'] == 'dsp']['mean'])
    test_error = float(data.df[data.df['metric_name'] == 'test_error']['mean'])
    logger.debug("dsp:{}",dsp,"test_error:{}",test_error,"cost:{}",cost)
    with open(output_path + 'dehb_run_{}.json'.format(args.seed), 'a+')as f:
        json.dump({'configuration': dict(cfg), 'dsp': dsp,
                   'test_error': test_error, 'num_epochs': budget,'cost':cost}, f)

        f.write("\n")
    return ({"cost":cost,
             "fitness": [dsp, test_error]})




try:
    modehb = MODEHB(objective_function=adult_objective_function,
                       cs=search_space,
